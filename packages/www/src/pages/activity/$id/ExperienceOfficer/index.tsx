...
function ExperienceOfficer({ activityDetailsPage }: ExperienceOfficerProps) {
  ...
  const [$activated, set$activated, get$activated] = FUtil.Hook.useGetState<number>(1);
  ...
  React.useEffect(() => {
    ...
    // console.log(info2, 'info2sidofjldkjfljsld');
    if (info5 && info5.top <= 162) {
      set$activated(5);
    } else if (info4 && info4.top <= 162) {
      set$activated(4);
    } else if (info3 && info3.top <= 162) {
      set$activated(3);
    } else if (info2 && info2.top <= 162) {
      set$activated(2);
    } else {
      set$activated(1);
    }
