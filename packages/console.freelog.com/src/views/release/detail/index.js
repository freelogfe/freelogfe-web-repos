...
export default {
    ...
    methods: {
        ...
        async resolveReleaseDetail(data) {
            ...
            document.title = this.release.releaseName + ' - Freelog';
                ...
                if (res != null) {
                    ...
                    for (let i = 0; i < arr.length; i++) {
                        ...
                        arr[i].policies = arr[i].policies.map(p => {
                            ...
                            this.pCombinationIDMap[p.pCombinationID] = {
                                releaseName: arr[i].releaseName,
        ...
        formatReleaseData() {
            this.release.policies = this.release.policies.filter(p => p.status === 1).map(p => {
                ...
                this.pCombinationIDMap[p.pCombinationID] = {
                    releaseName: this.release.releaseName,
                    policyName: p.policyName
                }
                return p
            })
