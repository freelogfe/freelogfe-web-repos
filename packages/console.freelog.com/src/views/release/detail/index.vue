  ...
  <div class="release-detail-wrapper" v-if="release">
    <div class="r-d-w-header clearfix">
      <div class="r-d-w-main-content">
        <el-alert class="r-d-w-waring" type="warning" :closable="false" v-if="!isOnline">
          <div slot="title">
            {{$t('release.warings[0]')}}
            <router-link :to="`/release/edit/${release.releaseId}`" v-if="isOwnRelease">{{$t('release.linkBtns.editRelease')}}</router-link>
          </div>
        </el-alert>
        ...
        <div class="cont">
          ...
          <div class="r-d-w-info">
            ...
            <span class="r-i-id">{{$t('release.releaseID')}} {{release.releaseId}}</span>
          </div>
    ...
    <div class="r-d-w-auth-box">
      <div class="r-d-w-main-content">
        <div class="r-d-w-node-list">
          <h3>{{$t('release.steps[0]')}}<a :title="$t('release.titles[0]')" class="rdw-w-create-node" href="/node/create" target="_blank" v-if="nodes.length"><i class="el-icon-plus"></i></a></h3>
          <el-select
            class="r-d-node-select"
            :class="{ 'highlight': checkedNodeId === '' }"
            :placeholder="$t('release.selectionPlaceholder')"
            v-model="checkedNodeId"
            v-if="nodes.length">
            <el-option
              v-for="node in nodeSelections"
              :key="node.nodeId"
              :label="node.nodeName + (node.isSigned ? `（${$t('release.iconTexts[1]')}）` : '')"
              :value="node.nodeId">
            </el-option>
          </el-select>
          <div class="rdwr-no-nodes" v-else>
            <el-alert type="warning" show-icon :closable="false">
              <div class="" slot="title">
                {{$t('release.warings[2]')}}<a href="/node/create" target="_blank">{{$t('release.linkBtns.createNode')}}</a>
              </div>
            </el-alert>
          </div>
        </div>
        <div class="r-d-w-policy-box " :class="{'highlight': checkedNodeId!=''}" v-loading="isShowContentLoading">
          <h3>
            {{$t('release.steps[1]')}}
            <!-- <span><i class="el-icon-top"></i> {{$t('release.iconTexts[0]')}}</span> -->
          </h3>
          <div class="rdw-p-scheme-box">
            <div class="rdw-p-left-box">
              <h4>{{$t('release.titles[1]')}}</h4>
              <release-depend-item
              ...
              <h4 v-if="baseUpcastReleasesList.length">{{$t('release.titles[2]')}}</h4>
              <release-depend-item
          ...
          <div class="rdw-p-auth-btn-bar" v-if="!checkedNode.isSigned">
            <el-button type="primary" class="rdw-p-auth-btn" @click="showSignBox">{{$t('release.btns.getAuth')}}</el-button>
          </div>
    ...
    <div class="r-d-w-main-content">
      <div class="r-d-w-description">
        <h2>{{$t('release.titles[3]')}}</h2>
        <div class="ql-snow">
          <div v-if="resourceDetail.resourceInfo.description"
               class="ql-editor"
               v-html="resourceDetail.resourceInfo.description"></div>
          <div class="" v-else>
            {{$t('release.noDescription')}}
          </div>
        </div>
      </div>
    </div>
    <!-- <el-dialog :title="$t('release.policiesComparison')" width="740px" :visible.sync="compareDialogVisible" v-if="selectedRelease.policies.length > 1">
      <policies-compare :selectedRelease="selectedRelease"></policies-compare>
    </el-dialog> -->
    <el-dialog :title="$t('release.titles[4]')" width="640px" :visible.sync="signDialogVisible" center>
      <signed-confirm
