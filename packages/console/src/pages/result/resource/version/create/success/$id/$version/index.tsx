    ...
    <div className={styles.modal}>
      {
        gotoState !== 0 && (<>
          ...
          <FTipText
            type="second"
            text={fI18nNext.t('version_created_successfully', {VersionNumber: match.params.version})}
          />
        </>)
      }
      ...
      }
      {
        gotoState === 2 && (<div className={styles.goto2}>
          <FTipText type="third" text={fI18nNext.t('jump_to_version_edit', {timer: count})}/>
          <div style={{width: 10}}/>
          <FTextBtn
            // theme={'primary'}
            onClick={gotoVersionInfo}
          >{fI18nNext.t('jump_now')}</FTextBtn>
        </div>)
      }
    </div>
  </FCenterLayout>)
