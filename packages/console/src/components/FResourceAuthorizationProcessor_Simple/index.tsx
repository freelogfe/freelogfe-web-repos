...
interface FResourceAuthorizationProcessor_Simple_Props {
  // resourceID: string;
...
interface FResourceAuthorizationProcessor_Simple_States {
  // licenseeResource: {
  //   resourceID: string;
  //   resourceName: string;
...
const initStates: FResourceAuthorizationProcessor_Simple_States = {
...
function FResourceAuthorizationProcessor_Simple({
  ...
  const [targetInfos, set_targetInfos, get_targetInfos] = FUtil.Hook.useGetState<FResourceAuthorizationProcessor_Simple_States['targetInfos']>(initStates['targetInfos']);
  const [activatedTarget, set_activatedTarget, get_activatedTarget] = FUtil.Hook.useGetState<FResourceAuthorizationProcessor_Simple_States['activatedTarget']>(initStates['activatedTarget']);
  ...
  async function addTargets(targetsFrom: Target[]): Promise<{ err: string }> {
    ...
    const result: FResourceAuthorizationProcessor_Simple_States['relations'] = needAddResources.map((t) => {
  ...
  async function _syncTargetInfo() {
    ...
    let targetInfos: FResourceAuthorizationProcessor_Simple_States['targetInfos'] = [];
    ...
    if (needAddResourceIDs.length > 0) {
      ...
      const resourceTargetInfos: FResourceAuthorizationProcessor_Simple_States['targetInfos'] = await _batchHandleResources({
    ...
    if (needAddObjectIDs.length > 0) {
      ...
      const objTargetInfos: FResourceAuthorizationProcessor_Simple_States['targetInfos'] = data_objs.map((o) => {
        return {
          targetID: o.objectId,
  ...
  async function removeTarget(target: Target): Promise<{ err: string }> {
    const result: FResourceAuthorizationProcessor_Simple_States['relations'] = get_relations().filter((r) => {
  ...
  async function getAllResourcesWithPolicies(): Promise<{
    resourceID: string;
    resourceName: string;
    policyIDs: string[];
  }[]> {
...
export default FResourceAuthorizationProcessor_Simple;
...
type I_batchHandleResources_Return = FResourceAuthorizationProcessor_Simple_States['targetInfos'];
...
async function _batchHandleResources({
  ...
  const resourceTargetInfos: FResourceAuthorizationProcessor_Simple_States['targetInfos'] = data_batchResourceInfo.map((r) => {
    let error: FResourceAuthorizationProcessor_Simple_States['targetInfos'][number]['error'] = '';
    let warning: FResourceAuthorizationProcessor_Simple_States['targetInfos'][number]['warning'] = '';
...
export async function getProcessor_simple(processorIdentifier: string): Promise<Processor> {
  while (true) {
